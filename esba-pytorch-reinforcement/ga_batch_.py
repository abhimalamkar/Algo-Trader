...
from environ import StocksEnv, DEFAULT_WINDOW_SIZE
...
HIDDEN_SIZE = 512
dataDir = "../models/"+str(HIDDEN_SIZE)+"x" + str(DEFAULT_WINDOW_SIZE) +"-" + NAME
...
class Net(nn.Module):
    def __init__(self, obs_size, act_size, hid_size=HIDDEN_SIZE):
...
def evaluate_batch(envs, net, device="cpu"):
    ...
    while len(done_set) < count:
        ...
        for i in range(count):
            if i in done_set:
                continue
            new_o, r, done, _ = envs[i].step(np.argmax(out[i]))
            obs[i] = new_o
            rewards[i] += r
            steps[i] += 1
            if done:
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(comment="-coinbase_hourly-ga-batch-" + str(DEFAULT_WINDOW_SIZE) + "-hidden-" + str(HIDDEN_SIZE) + "-population-" + str(POPULATION_SIZE))
    ...
    while True:
        ...
        if load_population:
            population += load("../models/"+str(HIDDEN_SIZE)+"x" + str(DEFAULT_WINDOW_SIZE) +"-" +"PLUG"+'/population.data')
            print("../models/"+str(HIDDEN_SIZE)+"x" + str(DEFAULT_WINDOW_SIZE) +"-" +"PLUG"+'/population.data')
            load_population = False
        ...
        elite = population[0]
        # net = build_net(getEnv(),elite[0])
        # torch.save(net.state_dict(), dataDir+'/%d_elite_net.model'%elite[1])

        save(dataDir+'/elite.data',elite)
        save(dataDir+'/population.data',population)
